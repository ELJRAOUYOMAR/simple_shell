...
int shell_myexit(shell_info_t *info)
{
    ...
    if (info->argc > 1)  /* If there is an exit argument */
    {
...
int shell_mycd(shell_info_t *info)
{
    ...
    if (info->argc == 1 || (info->argc == 2 && shell_strcmp(info->argv[1], "~") == 0))
    {
    ...
    else if (shell_strcmp(info->argv[1], "-") == 0)
    {
        ...
        shell_puts(shell_getenv(info, "OLDPWD"));
        shell_putchar('\n');
        change_dir_ret = /* TODO: handle this case properly */
...
/**
 * shell_myhelp - prints a help message
 * @info: Structure containing potential arguments. Used to maintain
 *          constant function prototype.
 *  Return: Always 0
 */
