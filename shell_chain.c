...
int shell_is_chain(shell_info_t *info, char *buf, size_t *p)
{
    ...
    if (buf[j] == '|' && buf[j + 1] == '|')
    {
        buf[j] = 0;
        j++;
        info->cmd_buf_type = SHELL_CMD_OR;
    }
    else if (buf[j] == '&' && buf[j + 1] == '&')
    {
        buf[j] = 0;
        j++;
        info->cmd_buf_type = SHELL_CMD_AND;
    }
    else if (buf[j] == ';') /* found end of this command */
    {
        buf[j] = 0; /* replace semicolon with null */
        info->cmd_buf_type = SHELL_CMD_CHAIN;
    }
    else
        return (0);
    *p = j;
...
/**
 * shell_check_chain - checks if we should continue chaining based on last status
 * @info: the parameter struct
 * @buf: the char buffer
 * @p: address of current position in buf
...
void shell_check_chain(shell_info_t *info, char *buf, size_t *p, size_t i, size_t len)
{
    ...
    if (info->cmd_buf_type == SHELL_CMD_AND)
    {
        if (info->status)
        {
            buf[i] = 0;
    ...
    if (info->cmd_buf_type == SHELL_CMD_OR)
    {
        if (!info->status)
        {
            buf[i] = 0;
...
/**
 * shell_replace_alias - replaces an alias in the tokenized string
 * @info: the parameter struct
 *
...
/**
 * shell_replace_vars - replaces variables in the tokenized string
 * @info: the parameter struct
 *
 * Return: 1 if replaced, 0 otherwise
 */
