    ...
    if (!*length) /* if nothing left in the buffer, fill it */
    {
        ...
        *buffer = NULL;
        ...
        if (bytes_read > 0)
        {
            ...
            /* if (shell_strchr(*buffer, ';')) is this a command chain? */
            {
                *length = bytes_read;
                info->cmd_buf = buffer;
            }
 ...
 * Return: bytes read
 */
ssize_t obtain_input(shell_info_t *info)
{
    ...
    if (length)    /* we have commands left in the chain buffer */
    {
        ...
        shell_check_chain(info, buffer, &end, start, length);
        while (end < length) /* iterate to semicolon or end */
        {
            if (shell_is_chain(info, buffer, &end))
                break;
            end++;
        }
        ...
        if (start >= length) /* reached end of buffer? */
        {
            ...
            info->cmd_buf_type = SHELL_CMD_NORM;
        }

        *buffer_pointer = pointer; /* pass back pointer to current command position */
        return (shell_strlen(pointer)); /* return length of current command */
    }
 ...
 * Return: bytes read
 */
ssize_t read_buffer(shell_info_t *info, char *buffer, size_t *size)
{
    ssize_t bytes_read = 0;

    if (*size)
        return (0);
    bytes_read = read(info->readfd, buffer, SHELL_READ_BUF_SIZE);
    if (bytes_read >= 0)
        *size = bytes_read;
    return (bytes_read);
}
...
{
    static char buffer[SHELL_READ_BUF_SIZE];
    ...
    character = shell_strchr(buffer + start, '\n');
    k = character ? 1 + (unsigned int)(character - buffer) : end;
    new_pointer = shell_realloc(pointer, chars_read, chars_read ? chars_read + k : k + 1);
    if (!new_pointer) /* MALLOC FAILURE! */
        return (pointer ? free(pointer), -1 : -1);

    if (chars_read)
        shell_strncat(new_pointer, buffer + start, k - start);
    else
        shell_strncpy(new_pointer, buffer + start, k - start + 1);
